Функция СведенияОВнешнейОбработке() Экспорт
  ...
  ОМДополнительныеОтчетыИОбработкиКлиентСервер = ПолучитьОбщийМодуль("ДополнительныеОтчетыИОбработкиКлиентСервер");
  ПараметрыРегистрации.Вставить("Вид", ОМДополнительныеОтчетыИОбработкиКлиентСервер.ВидОбработкиДополнительнаяОбработка());
  ...
  ДобавитьКоманду(ТаблицаКоманд,
          ...
          ОМДополнительныеОтчетыИОбработкиКлиентСервер.ТипКомандыВызовСерверногоМетода());
  
  ДобавитьКоманду(ТаблицаКоманд,
  		"Настройка выгрузки ЖР в Elastic",
  		"ОткрытьФормуВыгрузкиЖРвElastic",
  		ОМДополнительныеОтчетыИОбработкиКлиентСервер.ТипКомандыОткрытиеФормы());
  ПараметрыРегистрации.Вставить("Команды", ТаблицаКоманд);
  Возврат ПараметрыРегистрации;
КонецФункции

// Функция - Получить общий модуль
//
// Параметры:
//  ИмяМодуля	 - Строка	 - имя общего модулья
// 
// Возвращаемое значение:
//  Общий модуль, неопределено - общий модуль, при его обнаружении
//
Функция ПолучитьОбщийМодуль(ИмяМодуля)
...
Функция ОтправитьДанныеЗаПериод(настройки, табЗаписейЖР)
	Попытка
		Если табЗаписейЖР.Количество() > 0 Тогда
			...
			Для Каждого стрЖР Из табЗаписейЖР Цикл
				Если ТипЗнч(стрЖР.ПредставлениеМетаданных) = Тип("Строка") И Найти(стрЖР.ПредставлениеМетаданных, "Регистр") = 1 И стрЖР.Уровень = УровеньЖурналаРегистрации.Информация Тогда
					Продолжить;
				КонецЕсли;
				//отправляем каждые 1000 записей в logstash		
				ДобавитьСтрокуЖРвJSONЗапрос(JSONЗапрос, стрЖР);
				Если Сч > 0 И (Сч % 1000) = 0 Тогда		
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации("_Elastic_", 
			УровеньЖурналаРегистрации.Ошибка, ,
			"Логи ЖР. Ошибка выгрузки:" + ТекстОшибки, 
			СтрокаСоединенияИнформационнойБазы()
		);
		ВызватьИсключение;
	КонецПопытки;
